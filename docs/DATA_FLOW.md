...
- In hosted baseline and TLS ingress deployment, canonical telemetry topic configuration is `oncovax/telemetry` via environment settings (`MQTT_TOPIC`).
...
In TLS ingress topology, these operational surfaces are served behind nginx route protection.
...
In TLS ingress topology, Grafana is a protected operational surface behind nginx basic-auth host routing.
...
The implemented flow should be treated as a self-hosted engineering baseline with explicit scope and maturity boundaries.
